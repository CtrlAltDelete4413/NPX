...
GeneralEntries[#GeneralEntries+1] = {
  ...
  isEnabled = function(pEntity)
    if not (isPolice or isDoc) then return false end
...
GeneralEntries[#GeneralEntries+1] = {
  ...
  isEnabled = function(pEntity, pContext)
    return not IsDisabled() and (isPolice or isJudge) and not IsPedInAnyVehicle(PlayerPedId()) and (pEntity and pContext.flags["isVehicleSpawner"] or not pEntity and exports["np-vehicles"]:IsOnParkingSpot(PlayerPedId(), true))
  end
}
